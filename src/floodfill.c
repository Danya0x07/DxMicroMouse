...
void Floodfill_Setup(struct MazeCell goalCell, bool extendGoalCell)
{
...
bool Floodfill_RecomputeFromCell(struct MazeCell cell)
{
    ...
    while (!Queue_IsEmpty(&cellQueue) && !retcode) {
        ...
        distance = Maze_ReadCellMetadata(cell);
        minDistance = Floodfill_FindMinimumOpenNeighbor(cell, NULL);
...
struct MazeCell Floodfill_NextCell(struct MazeCell currentCell)
{
    ...
    Floodfill_FindMinimumOpenNeighbor(currentCell, &nextCell);
    return nextCell;
}

uint_fast16_t Floodfill_GetDistance(struct MazeCell cell)
{
